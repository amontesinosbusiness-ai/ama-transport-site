import { ArrowRight, Globe2, PackageSearch, PlaneTakeoff, Warehouse, Truck } from "lucide-react";
      ...
      <section className="section-padding relative">
        <div className="container grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-slate-300 max-w-xl">
              <div className="glass p-4 rounded-2xl card-hover">
          ...
          <div className="relative">
            <div className="absolute -inset-8 rounded-3xl bg-glow blur-3xl" aria-hidden />
            <div className="glass rounded-3xl p-8 border border-white/10 relative overflow-hidden">
              ...
              <div className="relative space-y-6">
                <p className="text-xs uppercase tracking-[0.25em] text-cyan-200/80">Project Control Tower</p>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {services.slice(0, 4).map((service) => (
                    <div key={service.title} className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <p className="text-white font-medium">{service.title}</p>
                      <p className="text-slate-400 text-xs">{service.copy}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section className="section-padding">
        <div className="container space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {industries.map((item) => (
              <div key={item} className="glass p-5 rounded-2xl flex items-center justify-between card-hover">
        ...
        <div className="container grid gap-10 lg:grid-cols-[1.2fr_0.8fr] items-center">
          ...
          <div className="glass rounded-3xl p-8 border border-white/10 card-hover">
            ...
            <p className="text-slate-300 text-sm leading-relaxed">
              Multi-vendor FF&E programs require a single control tower that tracks every shipment, receives and labels inventory in Miami, consolidates loads, and schedules final mile delivery precisely when jobsites can receive. AMA removes the guesswork so openings stay on schedule.
            </p>
          </div>
        </div>
      </section>
